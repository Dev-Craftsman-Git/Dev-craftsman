    ...
    return (
        <div className="p-8 w-full max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between md:items-center gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-bold text-[var(--text-primary)]">Theme Manager</h1>
                    <p className="text-[var(--text-secondary)]">Control the visual appearance of the website.</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    >
                        <div className="p-6 h-full bg-[var(--card-bg)] flex flex-col items-center text-center justify-center min-h-[200px]">
                            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center mb-4">
                                <Clock className="w-8 h-8 text-white" />
                            </div>
                            <h3 className="text-xl font-bold text-[var(--text-primary)] mb-2">Daily Rotation</h3>
                            <p className="text-sm text-[var(--text-secondary)]">
                                Automatic
                            </p>
                            {activeThemeId === 'daily' && (
                                <div className="absolute top-4 right-4 bg-[var(--primary)] text-white rounded-full p-1">
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-[var(--card-bg)] border border-[var(--border-color)] w-full max-w-2xl rounded-xl shadow-2xl flex flex-col max-h-[90vh]">
                        <div className="p-6 border-b border-[var(--border-color)] flex justify-between items-center">
                            <h2 className="text-xl font-bold text-[var(--text-primary)]">Create Custom Theme</h2>
                            <button onClick={() => setIsModalOpen(false)}><X className="w-6 h-6 text-[var(--text-secondary)]" /></button>
                        </div>

                        <div className="p-6 overflow-y-auto space-y-6">
                            <div>
                                <label className="block text-sm font-medium text-[var(--text-secondary)] mb-1">Theme Name</label>
                                <input
                                    type="text"
                                    value={newTheme.name}
                                    onChange={(e) => setNewTheme({ ...newTheme, name: e.target.value })}
                                    className="w-full bg-[var(--background)] border border-[var(--border-color)] rounded-lg p-2 text-[var(--text-primary)]"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {Object.entries(newTheme.colors).map(([key, value]) => (
                                    <div key={key}>
                                        <label className="block text-xs font-medium text-[var(--text-secondary)] mb-1 capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</label>
                                        <div className="flex gap-2">
                                            ...
                                            <input
                                                ...
                                                className="flex-1 bg-[var(--background)] border border-[var(--border-color)] rounded-lg px-2 text-sm text-[var(--text-primary)] font-mono"
                                            />
                                        </div>
                                    </div>
                                ))}
                            ...
                            {/* Preview */}
                            <div className="mt-4 p-4 rounded-lg border border-[var(--border-color)]" style={{ backgroundColor: newTheme.colors.background }}>
        ...
        >
            <div className="h-32 w-full relative" style={{ background: gradient }}>
                ...
                {isCustom && onDelete && (
                    <button
                        onClick={onDelete}
                        className="absolute top-2 right-2 p-1.5 bg-black/30 hover:bg-red-500 text-white rounded-full backdrop-blur-sm transition-colors opacity-0 group-hover:opacity-100"
                        title="Delete Theme"
                    >
                ...
                <div className="flex justify-between items-center mb-4">
                    <div className="flex -space-x-2">
                        {[colors.primary, colors.secondary, colors.accent].map((color: string, i: number) => (
                            <div
                                key={i}
                                className="w-8 h-8 rounded-full border-2 border-[var(--card-bg)]"
                                style={{ backgroundColor: color }}
                            />
                        ))}
                    </div>
                    {isActive && (
                        <span className="text-xs font-bold text-[var(--primary)] bg-[var(--primary)]/10 px-2 py-1 rounded-full flex items-center gap-1">
